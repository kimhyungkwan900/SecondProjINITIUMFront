...
import { fetchEmployeeByNo, updateMyInfo } from "../../../api/user/auth/employeesApi"; // Changed from studentsApi
...
export default function EmployeeUpdateMyInfo() { // Changed component name
    // --- 신규 상태: 비밀번호 확인 관련 ---
    ...
    const [phoneForm, setPhoneForm] = useState({ phone: "" }); // Added phone form
    const [pwdForm, setPwdForm] = useState({ currentPassword: "", newPassword: "", confirmPassword: "" });
    const [acctForm, setAcctForm] = useState({ bankCd: "", accountHolder: "", accountNumber: "" });
    const [saving, setSaving] = useState({ email: false, phone: false, pwd: false, acct: false }); // Added phone saving state
    const [feedback, setFeedback] = useState({ email: null, phone: null, pwd: null, acct: null }); // Added phone feedback state

    const PWD_REGEX = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[$@$!%*#?&])[A-Za-z\d$@$!%*#?&]{8,}$/;
    const PHONE_REGEX = /^01(?:0|1|[6-9])-(?:\d{3}|\d{4})-\d{4}$/; // Added phone regex
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const employeeNo = current.userId ?? current.loginId; // Changed studentNo to employeeNo
                const employee = await fetchEmployeeByNo(employeeNo); // Changed fetchStudentByNo to fetchEmployeeByNo
                if (!mounted) return;

                setEmailForm({ email: employee?.empEmail ?? employee?.email ?? "" }); // Changed stdntEmail to empEmail
                setPhoneForm({ phone: employee?.empPhone ?? employee?.phone ?? "" }); // Added phone form
                setAcctForm({
                    bankCd: employee?.bankCd ?? employee?.bankCode ?? "",
    ...
    const onChangePhone = (e) => setPhoneForm({ phone: e.target.value }); // Added phone change handler
    ...
    const phoneValid = useMemo(() => PHONE_REGEX.test(phoneForm.phone), [phoneForm.phone]); // Added phone validation
    ...
    const employeeNo = me?.userId ?? me?.loginId; // Changed studentNo to employeeNo

    const submitEmail = async (e) => {
        e.preventDefault();
        if (!emailValid) return alert("올바른 이메일을 입력하세요.");
        if (!employeeNo) return alert("사용자 정보가 없습니다. 다시 로그인해주세요."); // Changed studentNo to employeeNo
        
        setSaving((s) => ({ ...s, email: true }));
        setFeedback(f => ({ ...f, email: null }));
        try {
            await updateMyInfo(employeeNo, { email: emailForm.email }); // Changed updateMyInfo to updateEmployeeMyInfo
    ...
    const submitPhone = async (e) => { // Added submit phone function
        e.preventDefault();
        if (!phoneValid) return alert("올바른 전화번호를 입력하세요. (예: 010-1234-5678)");
        if (!employeeNo) return alert("사용자 정보가 없습니다. 다시 로그인해주세요.");
        
        setSaving((s) => ({ ...s, phone: true }));
        setFeedback(f => ({ ...f, phone: null }));
        try {
            await updateMyInfo(employeeNo, { phone: phoneForm.phone });
            setFeedback(f => ({ ...f, phone: { type: 'success', message: "전화번호가 수정되었습니다." }}));
        } catch (err) {
            console.error(err);
            const message = err?.response?.data?.message ?? "전화번호 수정 실패";
            setFeedback(f => ({ ...f, phone: { type: 'error', message }}));
        } finally {
    ...
    const submitAccount = async (e) => {
        ...
        try {
            await updateMyInfo(employeeNo, { // Changed updateMyInfo to updateEmployeeMyInfo
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8">
            <PageHeader
                title="개인정보 수정"
                breadcrumb={[
                    { label: "마이페이지(직원)", link: "/mypage" }, // Changed breadcrumb
                ...
                <>
                    ...
                    <section className="bg-white rounded-lg shadow-sm p-6">
                        ...
                        <form onSubmit={submitEmail} className="space-y-3">
                            <div className="flex gap-3 items-start">
                                ...
                                <button type="submit" disabled={!emailValid || saving.email || !employeeNo} className="px-5 py-2 rounded-md bg-[#354649] text-[#E0E7E9] hover:bg-[#6C7A89] disabled:opacity-50">
                                    {saving.email ? "저장중..." : "저장"}
                                </button>
                            </div>
                    ...
                    <section className="bg-white rounded-lg shadow-sm p-6">
                        ...
                        <form onSubmit={submitPhone} className="space-y-3">
                            <div className="flex gap-3 items-start">
                                <div className="flex-1">
                                    <TextInput type="text" placeholder="전화번호 입력 (예: 010-1234-5678)" value={phoneForm.phone} onChange={onChangePhone} />
                                    {!phoneValid && phoneForm.phone && <p className="text-xs text-red-500 mt-1">올바른 전화번호 형식을 입력하세요. (예: 010-1234-5678)</p>}
                                </div>
                                <button type="submit" disabled={!phoneValid || saving.phone || !employeeNo} className="px-5 py-2 rounded-md bg-[#354649] text-[#E0E7E9] hover:bg-[#6C7A89] disabled:opacity-50">
                                    {saving.phone ? "저장중..." : "저장"}
                                </button>
                            </div>
                            <FeedbackMessage feedback={feedback.phone} />
                        </form>
                    </section>
                    ...
                    <section className="bg-white rounded-lg shadow-sm p-6">
                        ...
                        <form onSubmit={submitAccount} className="space-y-3">
                            ...
                            <div className="flex justify-between items-center pt-2">
                                ...
                                <button type="submit" disabled={!!acctError || saving.acct || !employeeNo} className="px-5 py-2 rounded-md bg-[#354649] text-[#E0E7E9] hover:bg-[#6C7A89] disabled:opacity-50 ml-auto">
                                    {saving.acct ? "저장중..." : "저장"}
                                </button>
                            </div>
                        </form>
