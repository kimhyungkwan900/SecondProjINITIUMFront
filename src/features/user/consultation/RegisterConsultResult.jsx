const RegisterConsultResult = ({info, onClose, setRefreshKey})=>{
    return(
        <div class="p-6 border border-gray-300">
            <table class="w-full table-fixed border-collapse">
                ...
                <tr>
                    ...
                    <td class="border border-gray-300 px-4 py-2">{`${info.dscsnApplyDto.dscsnScheduleDto.scheduleDate.slice(0,4)}-${info.dscsnApplyDto.dscsnScheduleDto.scheduleDate.slice(4,6)}-${info.dscsnApplyDto.dscsnScheduleDto.scheduleDate.slice(6)}`}</td>
                </tr>

                <tr>
                    <th class="bg-gray-200 border border-gray-300 px-4 py-2 text-left">상담시간</th>
                    <td class="border border-gray-300 px-4 py-2">{`${info.dscsnApplyDto.dscsnScheduleDto.startTime.slice(0,2)}:${info.dscsnApplyDto.dscsnScheduleDto.startTime.slice(2)}`}</td>
                    <th class="bg-gray-200 border border-gray-300 px-4 py-2 text-left">상담장소</th>
                    <td class="border border-gray-300 px-4 py-2">상담실</td>
                </tr>

                <tr>
                    <th class="bg-gray-200 border border-gray-300 px-4 py-2 text-left">상담자</th>
                    <td class="border border-gray-300 px-4 py-2">{info.dscsnApplyDto.dscsnScheduleDto.empName}</td>
                </tr>

                <tr>
                    <th class="bg-gray-200 border border-gray-300 px-4 py-2 text-left">상담항목</th>
                    <td class="border border-gray-300 px-4 py-2">{info.dscsnKindDto.dscsnKindName}</td>
                    <th class="bg-gray-200 border border-gray-300 px-4 py-2 text-left">공개여부</th>
                    <td class="border border-gray-300 px-4 py-2">
                    <label class="inline-flex items-center mr-6">
                        <input type="radio" name="release" value="Y" class="h-4 w-4 text-blue-700 border-gray-300 focus:ring-blue-700"/>
                        <span class="ml-2">Y</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="radio" name="release" value="N" class="h-4 w-4 text-blue-700 border-gray-300 focus:ring-blue-700"/>
