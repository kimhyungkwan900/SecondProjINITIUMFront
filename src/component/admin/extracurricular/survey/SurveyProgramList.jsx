      ...
      <table className="w-full table-auto border-collapse border" style={{ tableLayout: 'fixed' }}>
        <thead className="bg-gray-100 text-center">
          <tr>
            ...
            <th className="border p-2" style={{ width: "15%" }}>교육종료일</th>
            <th className="border p-2" style={{ width: "10%" }}>등록된설문</th>
          </tr>
        </thead>
        <tbody>
          {programs.length === 0 ? (
            ...
            <>
              {programs.map((program) => (
                ...
                >
                  ...
                  <td className="border p-2">{program.eduEndDt?.replace("T", " ") || "-"}</td>
                  <td className="border p-2">{program.totalSurvey}</td>
                </tr>
              ))}
              {/* 빈 행 추가로 높이 맞추기 */}
