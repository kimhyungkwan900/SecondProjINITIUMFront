...
const ScoreLevelEditor = ({ scoreLevels = [], onChange }) => {
  ...
  const sortLevels = () => {
    const next = [...scoreLevels].sort(
      (a, b) => Number(a.minScore ?? 0) - Number(b.minScore ?? 0)
    );
    onChange(next);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-base font-semibold text-gray-700">점수 구간</h2>
        <div className="flex gap-2">
          <button type="button" onClick={addLevel} className="adm-btn adm-btn--secondary">
            구간 추가
          </button>
          <button type="button" onClick={sortLevels} className="adm-btn adm-btn--secondary">
            정렬
          </button>
        </div>
      </div>
      ...
      {scoreLevels?.length ? (
        scoreLevels.map((lv, idx) => {
          ...
          return (
            <div
              key={idx}
              className={`grid grid-cols-12 gap-3 items-start border border-gray-200 rounded-lg p-4 bg-white ${rowInvalid || rowOverlap ? "ring-1 ring-red-300" : ""
                }`}
            >
              {/* 최소점수 */}
              <TextInput
                ...
                className="col-span-12 md:col-span-2 adm-control w-full !md:w-full"
                aria-label={`레벨 #${idx + 1} 최소점수`}
              />
              ...
              <TextInput
                ...
                className="col-span-12 md:col-span-3 adm-control w-full !md:w-full"
                aria-label={`레벨 #${idx + 1} 이름`}
              />
              ...
              <TextInput
                ...
                className="col-span-12 md:col-span-4 adm-control w-full !md:w-full"
                aria-label={`레벨 #${idx + 1} 설명`}
              />
        ...
        <p className="adm-empty">점수 구간이 없습니다. “구간 추가”로 시작하세요.</p>
      )}
    </div>
  );
};
